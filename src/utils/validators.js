...
export const familySignupValidation = [
  ...
  passwordValidation,
  phoneValidation,
  body('alternatePhoneNo').optional().matches(/^\d{10}$/).withMessage('Alternate phone number must be exactly 10 digits'),
...
export const caregiverSignupValidation = [
  ...
  body('skills.*').isIn([
    'basic_care', 'medical_care', 'dementia_care', 'mobility_assistance', 
    'medication_management', 'companionship', 'meal_preparation', 'housekeeping',
    'personal_hygiene', 'transportation', 'emergency_response', 'physical_therapy'
...
export const elderValidation = [
  body('name').isLength({ min: 2, max: 50 }).withMessage('Elder name must be between 2 and 50 characters'),
  body('age').isInt({ min: 1, max: 120 }).withMessage('Age must be between 1 and 120'),
  body('address').isLength({ min: 5, max: 200 }).withMessage('Address must be between 5 and 200 characters'),
  body('phoneNo').optional().matches(/^\d{10}$/).withMessage('Phone number must be exactly 10 digits')
];

// Job post validation
export const jobPostValidation = [
  body('elderName').isLength({ min: 2, max: 50 }).withMessage('Elder name must be between 2 and 50 characters'),
  body('date').isISO8601().withMessage('Please provide a valid date'),
  body('startTime').matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).withMessage('Please provide valid time in HH:MM format'),
  body('durationHours').isFloat({ min: 0.5, max: 24 }).withMessage('Duration must be between 0.5 and 24 hours'),
  body('salary').isFloat({ min: 0 }).withMessage('Salary must be a positive number'),
  body('location').isLength({ min: 5, max: 200 }).withMessage('Location must be between 5 and 200 characters'),
  body('skillRequired').isArray({ min: 1 }).withMessage('At least one skill is required'),
  body('skillRequired.*').isIn([
    'basic_care', 'medical_care', 'dementia_care', 'mobility_assistance', 
    'medication_management', 'companionship', 'meal_preparation', 'housekeeping',
    'personal_hygiene', 'transportation', 'emergency_response', 'physical_therapy'
  ]).withMessage('Invalid skill provided')
